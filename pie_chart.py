...
def load_and_standardize_log(csv_path: Path) -> pd.DataFrame | None:
    """Loads a single CSV, standardizes columns, and infers protocol."""
    ...
    try:
        ...
        if file_name == "conn.log.csv" and "service" in df.columns:
            # For conn.log, use 'service' for application protocols
        ...
        if {"orig_bytes", "resp_bytes"}.issubset(df.columns):
            ...
            # For files.log, 'total_bytes' is the file size
...
def load_all_csvs(csv_dir: str | os.PathLike) -> pd.DataFrame:
    ...
    all_dfs = [df for csv_file in csv_dir_path.glob("*.csv") if (df := load_and_standardize_log(csv_file)) is not None]
...
def plot_protocol_pie(df: pd.DataFrame, out_path: Path | None = None) -> None:
    """Plot protocol share as a pie chart, grouping specific protocols."""
    ...
    # Parent categories to display
    ORDER = ["ldap", "dns", "rpc", "tls", "kerberos", "other"]

    # Map variants to parent categories
